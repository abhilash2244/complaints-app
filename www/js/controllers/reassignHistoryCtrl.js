...
.controller('reassignedHistory', function ($scope, $rootScope, commonService, $ionicPopup, $state, formsSave, alertService, strings, config, $localstorage, $ionicModal, $cordovaSQLite, $ionicListDelegate) {
			...
			if (len == 0) {
				...
				formsSave.saveReassignedForm(url, obj, securityHeaders, function (response) {
					if (response.status) {
						...
						alertService.doAlert(strings.invalidresponse, function (res) {
							$ionicListDelegate.closeOptionButtons();
						});
					} else {
						$rootScope.signatureimage = "";
						commonService.Loaderhide();
						/*var query = "DELETE FROM ReassignFormsList WHERE FormId=?";
						$cordovaSQLite.execute(db, query, [item.FormId]).then(function (res) {}, function (err) {
						alert(JSON.stringify(err));
						});*/
