...
app.service('mapComponentServices',['$rootScope','commonService','strings','$http','$ionicModal','setGetObj','$ionicPopover','loginService','$localstorage','alertService','$ionicPopup','config','$state','$ionicSideMenuDelegate','$ionicScrollDelegate','$location','formsService','$cordovaFileTransfer' ,function($rootScope,commonService,strings,$http,$ionicModal,setGetObj,$ionicPopover,loginService,$localstorage,alertService,$ionicPopup,config,$state,$ionicSideMenuDelegate,$ionicScrollDelegate,$location,formsService,$cordovaFileTransfer){
	var mapInServices ={};
	...
	Developer: Lokesh
	Date: 02-05-2018
	Description: fileOpener2 add for opening references*/
	mapInServices.goToReferences = function(item){
		// commonService.LoaderShow();
		// console.log('item '+JSON.stringify(item));
		// var headers = commonService.securityHeaders();
		// var url =  config.url + "/api/v1/gridFS/getAttachedReferenceOfForm/"+item._id;
		// console.log('url-------------- '+url)
		// console.log('item.contentType '+item.contentType)
		// var targetPath = cordova.file.externalDataDirectory + item.filename;
		// $cordovaFileTransfer.download(url, targetPath, {}, true).then(function (result) {
  //     		if(result.message == "Pdf Not Yet Generated "){
  //     			 alertService.showToast("File is empty..");
  //     		}
  //     		else{
  //     			cordova.plugins.fileOpener2.open(
		...
		// 		            	alertService.showToast("Supporting app is not available to open the file");
		// 		            }
		// 		        },
		// 		        success : function () {
		// 		        	commonService.Loaderhide();
  ...
  //     		console.log("download error source " + error.sourceImage);
...
//			alertService.doAlert('Please copy the attribute value in map by selecting feature');
	...
	mapInServices.setLocationFieldValue = function($scope,data,selectedFeatureData){
//		var selectedFeatureData = commonService.getCurrentSelectedFeature();
		if(selectedFeatureData != undefined){
			if ("data" in data){
				$scope.selectedFormRecordFields[data.data.id] = selectedFeatureData;
...
@Developer : SKG
@Date : 3/5/2018
@Description : getting ci=urrent location and fill the sorce field
*/
mapInServices.getCurrentLocationAndSetToSource = function($scope){
 		clearSource($scope);
		...
		commonService.getLatLong(function (geoLocation) {
			...
			if (geoLocation.netstatus == "success") {
				$scope.sourceForRouteBetween2Points = new L.marker([geoLocation.latlong[0], geoLocation.latlong[1]], {icon: greenIcon}).addTo($rootScope.map);
 	...
 	mapInServices.clearSource = function($scope){
 		clearSource($scope);
 	}

 	function clearSource($scope){
 	...
 	mapInServices.clearDestination = function($scope){
	...
	mapInServices.routeBetween2PointsTool = function($scope){
    	if(!$scope.routeBetween2PointsCheck.value){
			...
			$ionicModal.fromTemplateUrl('customization-TPCL/modified/templates/routeBetween2PointsTool.html', {
				scope: $scope,
				animation: 'slide-in-up'
			}).then(function (modal) {
...
//		closeellipsePopoverInForms($scope);
	...
	mapInServices.references = function($scope,data){
		...
		Developer: Lokesh
		Date: 02-05-2018
		Description: modalForReferences.html modified
		*/
		closeellipsePopoverInForms($scope);
		var headers = commonService.securityHeaders();
		var url =  config.url + "/api/v1/gridFS/getAllAttachedReferenceOfForm/"+commonService.getSelectedFormId();
		console.log('url  '+url)
		formsService.assignedtask(url, headers, function (response, status) {
		...
		$ionicPopover.fromTemplateUrl('customization-TPCL/modified/templates/modalForReferences.html', {
					scope: $scope
				}).then(function (modal) {
    ...
    function onMapClick(e,$scope){
				...
				else if($rootScope.enableClickToFillLocationField == true){
					$rootScope.$broadcast('fillLocationInProjectTaskForm', { fieldValue: e.latlng.lat+","+e.latlng.lng });
...
//					    popupAnchor:  [-3, -76] // point from which the popup should open relative to the iconAnchor
					...
					if($scope.sourceForRouteBetween2Points && $scope.destinationForRouteBetween2Points == undefined){
						$scope.destinationForRouteBetween2Points = new L.marker([e.latlng.lat, e.latlng.lng], {icon: greenIcon});
					    ...
					    $scope.destinationForRouteBetween2Points.addTo($rootScope.map);
					}else if($scope.sourceForRouteBetween2Points == undefined){
						$scope.sourceForRouteBetween2Points = new L.marker([e.latlng.lat, e.latlng.lng], {icon: greenIcon});
						$scope.source = $scope.sourceForRouteBetween2Points._latlng.lat + ',' + $scope.sourceForRouteBetween2Points._latlng.lng;

					    $scope.sourceForRouteBetween2Points.addTo($rootScope.map);
					}
...
app.service('deviceManagement',['loginService','commonService','$state','alertService','$localstorage','config','$ionicHistory',function(loginService,commonService,$state,alertService,$localstorage,config,$ionicHistory){
	...
	deviceManagementControl.logout = function(username,manufacturer,model,platform,uuid,version)
    {
